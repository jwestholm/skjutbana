...
- [ ] 1. Add RoundRecord dataclass and CSV enhancement to diagnostics.py
  - [ ] 1.1 Define `RoundRecord` dataclass in `src/engine/ai/diagnostics.py`
  ...
  - [ ] 1.2 Enhance `FunnelTracker.save_csv()` to accept optional `round_records` parameter
...
- [ ] 2. Add `candidate_limit` and `sampling_mode` to AIRuntime settings
  - [ ] 2.1 Modify `AIRuntime.__init__()` in `src/engine/ai/runtime.py`
...
- [ ] 3. Wire `candidate_limit` through bootstrap.py → hit_scanner.py
  - [ ] 3.1 Add `self.candidate_limit: int = 200` instance variable to `HitScanner.__init__()` in `src/engine/camera/hit_scanner.py`
    - Replace the hardcoded `candidates[:150]` slice in `_detect_frame_candidates` with `candidates[:self.candidate_limit]`
    - _Requirements: 6.3, 6.4_

  - [ ] 3.2 Modify `wrapped_update` in `src/engine/ai/bootstrap.py`
...
- [ ] 5. Refactor AITrainingScene to use RoundRecord list as single source of truth
  - [ ] 5.1 Replace `auto_stats` dict with `round_records` list in `src/engine/scenes/ai_training.py`
  ...
  - [ ] 5.2 Add `_build_round_record()` method to replace `_record_detection_stats()`
  ...
  - [ ] 5.3 Add round-ID state logging via `_log_round_state(round_id, state_name)`
  ...
  - [ ] 5.4 Add session summary logging at session end
...
- [ ] 6. Rewrite `_build_auto_report()` to compute all stats from `round_records`
  - [ ] 6.1 Implement core stats computation from `round_records`
  ...
  - [ ] 6.2 Add AI guess pre-facit statistics section
...
- [ ] 8. Add block statistics per 100 shots to report
  - [ ] 8.1 Implement block statistics computation in `_build_auto_report()`
...
- [ ] 9. Add candidate count statistics to report
  - [ ] 9.1 Implement candidate count statistics in `_build_auto_report()`
...
- [ ] 10. Add first-100 vs last-100 comparison to report
  - [ ] 10.1 Implement first-100 vs last-100 comparison in `_build_auto_report()`
...
- [ ] 11. Implement sampling mode functions
  - [ ] 11.1 Add four sampling mode functions to `src/engine/scenes/ai_training.py`
  ...
  - [ ] 11.2 Replace `_choose_auto_screen_point()` with sampling mode dispatch
...
- [ ] 12. Wire CSV export with round_records at session end
  - [ ] 12.1 Update session-end code to pass `round_records` to `FunnelTracker.save_csv()`

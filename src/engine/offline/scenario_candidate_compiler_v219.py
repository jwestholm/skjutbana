...
"""Compile generated V2.19 worlds into the existing V2.16 candidate-pack schema.

This is the bridge that makes seed-generated worlds useful to V2.17/V2.18 and
later overnight training without inventing a second candidate-dataset format.
"""
...
from .scenario_generator_v219 import OfflineScenarioGeneratorV219


class ScenarioCandidateCompilerV219:
    def __init__(
        self,
        *,
        generator: OfflineScenarioGeneratorV219,
        output_root: Path = Path("content/ai/candidate_synthetic_v219"),
    ...
    def compile(
        ...
        *,
        ...
        session_id = session_id or f"v219_{split}_{int(first_seed)}_{count}_{int(time.time())}"
        recorder = CandidateShadowRecorderV216(
            self.capture_config,
            background=f"v219_media_{split}",
            benchmark_seed=int(first_seed),
            sampling_mode="v219_generated_world",
        ...
        for offset in range(count):
            ...
            extra = {
                "v219_generated": True,
                "v219_scenario": scenario.spec.to_dict(),
                "known_holes_before_shot": [
                    {"camera_x": float(x), "camera_y": float(y)} for x, y in scenario.spec.known_holes
                ],
            ...
            if (offset + 1) % 10 == 0 or offset + 1 == count:
                print(f"V2.19 candidate compile: {offset + 1}/{count} worlds")
        ...
        report_path = Path(summary.get("root") or self.output_root / "sessions" / session_id) / "v219_compile_report.json"

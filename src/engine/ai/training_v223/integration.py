...
from .trainer_v2234 import schedule_cycle_v2234
...
def install_v2230_training_pipeline() -> None:
    ...
    def wrapped_report(self):
        ...
        try:
            ...
            if bool(getattr(self, "_v223_f2_started", False)) and shots >= 10 and not bool(getattr(self, "_v223_autotrain_scheduled", False)):
                ...
                started = schedule_cycle_v2234(session_id=session_id, quick=True)
                msg = f"V2.23.4: {shots} grupper sparade; proposal→rich→patch→ranker-cykel {'startad' if started else 'redan aktiv'} (shadow)."
                try:
    ...
    _INSTALLED = True
    print("[V2.23.4] patch NewHole model + candidate cascade installed (F2 autotrain shadow-only; live authority unchanged)")

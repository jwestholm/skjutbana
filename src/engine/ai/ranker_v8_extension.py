...
ROOT = Path("content/ai/ranking_v210")
...
def _write_status(error: str | None = None) -> None:
    try:
        ...
        temp.write_text(json.dumps(_METRICS, indent=2, ensure_ascii=False), encoding="utf-8")
...
def _rank_for_radius(candidates: Sequence[dict[str, Any]], gt: tuple[float, float], radius: float) -> int | None:
...
def _get_model(runtime: Any) -> RankerV8ShadowModel:
    model = getattr(runtime, "_ranker_v8_shadow", None)
    if isinstance(model, RankerV8ShadowModel):
        return model
    model = RankerV8ShadowModel()
    runtime._ranker_v8_shadow = model
    return model


def _write_shadow_row(runtime: Any, gt: tuple[float, float], model: RankerV8ShadowModel) -> None:
    pool = [dict(candidate) for candidate in getattr(runtime, "_v28_hypothesis_pool", []) or []]
    actual = [dict(candidate) for candidate in getattr(runtime, "_v28_actual_pool", []) or []]
    if not pool:
        return
    ...
    row = {
        ...
        "actual": {
            ...
            "selected_distance_px": _distance(actual[0], gt) if actual else None,
        },
        "v8_shadow": {
            "loaded": bool(model.loaded),
            "rank_10": _rank_for_radius(shadow, gt, 10.0) if shadow else None,
            "rank_20": _rank_for_radius(shadow, gt, 20.0) if shadow else None,
            "rank_42": _rank_for_radius(shadow, gt, 42.0) if shadow else None,
            "selected_distance_px": _distance(shadow[0], gt) if shadow else None,
            "model": model.summary(),
        },
    }
    final_path = session_dir / f"shot_{sequence:06d}.json"
    temp_path = session_dir / f".shot_{sequence:06d}.{os.getpid()}.tmp"
    temp_path.write_text(json.dumps(row, ensure_ascii=False) + "\n", encoding="utf-8")
...
def install_ranker_v8_extension(source: str = "unknown") -> None:
    """Install V2.10 V8 shadow comparison outside the V2.9 dataset writer."""
    global _INSTALLED
    ...
    if bool(getattr(AIRuntime, "_ranker_v8_extension_installed", False)):
        _INSTALLED = True
    ...
    def rank_with_funnel_wrapped(
        ...
        if gt_xy is not None:
            _METRICS["labelled_calls"] = int(_METRICS.get("labelled_calls", 0) or 0) + 1
            try:
                _write_shadow_row(self, (float(gt_xy[0]), float(gt_xy[1])), model)
    ...
    AIRuntime._ranker_v8_extension_installed = True
    _INSTALLED = True
    _METRICS["installed"] = True
    _METRICS["installed_at"] = time.time()
    _write_status()
    print(
        "[RANKER-V8] V2.10 monotonic optimizer shadow integration installed "
        f"(pid={os.getpid()} session={_RUNTIME_SESSION_ID} source={_METRICS.get('install_source')})"
    )


__all__ = ["install_ranker_v8_extension"]

...
class VideoPlayer:
    def __init__(self, path: str, target_size: tuple[int, int]) -> None:
        self.path = path
    ...
    def update(self, dt: float) -> pygame.Surface | None:
        ...
        # Förhindra spiral-of-death om dt är stort
        max_steps = 5
        steps = 0
    ...
    def _frame_to_surface(self, frame_bgr: np.ndarray) -> pygame.Surface:
        ...
        # pygame.surfarray.make_surface vill ha (w, h, 3)
        frame_rgb = np.transpose(frame_rgb, (1, 0, 2))
        surf = pygame.surfarray.make_surface(frame_rgb)

        if surf.get_size() != self.target_size:

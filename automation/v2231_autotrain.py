...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=22310)
    args = parser.parse_args()
    budget_s = max(0.01, args.hours) * 3600.0
    deadline = time.time() + budget_s
    round_no = 0
    print("V2.23.1 AUTONOMOUS CHALLENGER LOOP")
    print("=================================")
    ...
    while time.time() < deadline and (args.max_rounds <= 0 or round_no < args.max_rounds):
        ...
        if report.get("status") in {"insufficient_data", "insufficient_positive_support"}:
            print("[AUTOTRAIN] stopping: insufficient positive proposal support")
            break
